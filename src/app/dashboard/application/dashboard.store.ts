...
export class DashboardStore {
  ...
  /**
   * Appends a new notification to the list in real-time.
   */
  ...
  /**
   * Deletes a notification both client-side and server-side (if stored).
   */
  deleteNotification(id: string): void {
    // Optimistic UI update: remove immediately from signal
    this.notificationsSignal.update(current => current.filter(n => n.id !== id));

    if (id.startsWith('stored-')) {
      const dbId = id.replace('stored-', '');
      this.dashboardApi.deleteNotification(dbId).subscribe({
        next: () => console.log(`Deleted notification ${dbId} from DB`),
        error: (err) => console.error('Failed to delete notification from DB:', err)
      });
    }
  }
