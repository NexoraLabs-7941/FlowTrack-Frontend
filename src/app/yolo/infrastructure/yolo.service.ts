...
export class YoloService {
  ...
  getDashboardData(): Observable<YoloDashboardData> {
    return forkJoin({
      ...
      audits: this.getAforoCollection<YoloAudit>('yolo-audits', this.getLocalAudits()),
      peopleCounters: this.getAforoCollection<PeopleCounterRecord>('people-counters', this.getLocalPeopleCounters())
    });
  }
  ...
  getTraficoDiarioHoy(idCamara?: string): Observable<number> {
    const baseUrl = environment.aforoApiBaseUrl;
    const token = localStorage.getItem('token');
    const headers = token ? { Authorization: `Bearer ${token}` } : undefined;
    const today = new Date().toISOString().slice(0, 10);
    const params: any = { fechaInicio: today, fechaFin: today };
    if (idCamara) params.camaraId = idCamara;

    return this.http.get<any[]>(`${baseUrl}/analitica/trafico-diario`, { headers, params }).pipe(
      map(items => items.reduce((sum, item) => sum + (Number(item.totalIngresos) || 0), 0)),
      catchError(() => of(0))
    );
  }
  ...
  saveAudit(audit: YoloAudit): Observable<YoloAudit> {
    ...
    return this.http.post<YoloAudit>(`${baseUrl}/yolo-audits`, audit, { headers }).pipe(
  ...
  validateAudit(audit: YoloAudit, stock: StockResource[]): Observable<YoloAudit> {
    ...
    const persistAudit$ = this.http.put<YoloAudit>(`${baseUrl}/yolo-audits/${audit.id}`, validatedAudit, { headers }).pipe(
  ...
  savePeopleCounter(record: PeopleCounterRecord): Observable<PeopleCounterRecord> {
    ...
    return this.http.post<PeopleCounterRecord>(`${baseUrl}/people-counters`, record, { headers }).pipe(
  ...
  private getAforoCollection<T>(path: string, fallback: T[]): Observable<T[]> {
    const baseUrl = environment.aforoApiBaseUrl;
    const token = localStorage.getItem('token');
    const headers = token ? { Authorization: `Bearer ${token}` } : undefined;
    return this.http.get<T[]>(`${baseUrl}/${path}`, { headers }).pipe(
      map(items => Array.isArray(items) ? items : fallback),
      catchError(() => of(fallback))
    );
  }
